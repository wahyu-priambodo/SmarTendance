...
def validate_user_form(
  user_id:str, user_role:str, user_fullname:str, user_pw:str, user_confirm_pw:str, user_email_address:str, user_uid:str, user_home_address:str = None, 
  student_class:str = None, lecturer_major:str = None
  ) -> bool:
...
def admin_edit(id: str):
  ...
  if not id:
    return redirect(url_for('admin_ep.dashboard'))
  # Check if the id is valid
  # course = Course.query.filter_by(course_id=id).first()
  student = User.query.filter_by(user_id=id, user_role='STUDENT').first()
  lecturer = User.query.filter_by(user_id=id, user_role='LECTURER').first()
  
  if not (lecturer or student):
    return redirect(url_for('admin_ep.dashboard'))
  # Check if user edit course or user
  pass
